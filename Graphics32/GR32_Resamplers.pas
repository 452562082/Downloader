...
begin
  Result := SInc(Value * FWidthReciprocal); 
end;
...
begin
  ...
  tt := Sqr(Value);
  ttt := tt * Value;
  if Value < 1 then Result := (21 * ttt - 36 * tt + 16 ) * OneEighteenth  
...
begin
  Result := Exp(Sqr(Value) * FSigmaReciprocalLn2);       
end;
