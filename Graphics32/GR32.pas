...
uses
  {$IFDEF FPC} LCLIntf, LCLType, Types, Graphics,{$ELSE}
  Windows, Messages, Graphics,{$ENDIF}
...
{$ENDIF}
      ...
      if Supports(SrcBitmap.Backend, IFontSupport) then 
...
procedure TCustomBitmap32.Assign(Source: TPersistent);
  ...
  begin
    ...
    try
      AssignFromGraphicPlain(TempBitmap, SrcGraphic, clRed32, False); 
  ...
  begin
    ...
    if Supports(TargetBitmap.Backend, IFontSupport) then 
...
begin
  ...
  begin
    ...
    if Dx > 0 then
    begin
      if (X1 > Cx2) or (X2 < Cx1) then Exit; 
      Sx := 1;
    end
    else
    begin
      if (X2 > Cx2) or (X1 < Cx1) then Exit; 
      Sx := -1;
      X1 := -X1;   X2 := -X2;   Dx := -Dx;
      Cx1 := -Cx1; Cx2 := -Cx2;
      Swap(Cx1, Cx2);
    ...
    if Dy > 0 then
    begin
      if (Y1 > Cy2) or (Y2 < Cy1) then Exit; 
      Sy := 1;
    end
    else
    begin
      if (Y2 > Cy2) or (Y1 < Cy1) then Exit; 
      Sy := -1;
      Y1 := -Y1;   Y2 := -Y2;   Dy := -Dy;
      Cy1 := -Cy1; Cy2 := -Cy2;
      Swap(Cy1, Cy2);
