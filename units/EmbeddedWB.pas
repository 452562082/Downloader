...
{$ENDIF}
    ...
    function LoadFromString(const St: string): HRESULT; 
...
{$IFDEF USE_EwbTools}
    ...
    function SearchText(const Value: string; const iPos: Integer = 1): IHTMLTxtRange; 
...
procedure SetMouseWheelFixActive(const bActive:Boolean );
implementation
      ...
      begin
        ...
        begin
          ...
          if (Win32Platform = VER_PLATFORM_WIN32_NT) and (Win32MajorVersion > 4) then
            PostMessage(FindWindowEx(PopHandle, 0, 'Button', nil), BM_CLICK, 0, 0) 
          else
...
begin
  ...
  begin
    ...
    if not IsWindow(Result) then
      Result := GetIEWin('MacromediaFlashPlayerActiveX');
  end
  else
    Result := 0;
end;
...
begin
  ...
  begin
    ...
    if Supports(Unk, IWebBrowser2, Result) then 
...
function TEmbeddedWB.Wait(TimeOut: Longword = 0): Boolean; 
var
  StartTime: DWORD;
...
begin
  ...
  begin
    ...
    try
      FramesIndexList.CommaText := FrameList; 
...
begin
  try
    ...
    SendMessage(Self.GetIEWin('Shell DocObject View'), WM_COMMAND, 336 , 0);
  end;
end;
...
{$IFDEF Enable_AutoMouseWheelFix}
  if Assigned(EwbControl) and (EwbControl.MouseWheelFix.Active<>bActive) then
    EwbControl.MouseWheelFix.Active := bActive;
